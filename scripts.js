...
// Adicionando navegação (anterior/próximo) no visualizador de fotos
let currentIndex = 0;
...
function openPhotoViewer(galleryId, index) {
    ...
    const gallery = galleries[galleryId];
    currentIndex = index;
    viewerImage.src = gallery[currentIndex].src;
    viewerCaption.textContent = gallery[currentIndex].caption;
    ...
    photoViewer.appendChild(prevButton);
    photoViewer.appendChild(nextButton);
    photoViewer.style.display = 'block';
}